...
import { TetaUserError, sqlRenderer, aggregate, join, select, eq, group, loop, toSql, count } from "../mod.ts";
...
describe("error paths", () => {
    ...
    test("rejects loop steps with mismatched column names", () => {
        const users = createUsersTable();
        const orders = createOrdersTable();
        const base = select(users, (user) => ({ id: user.id }));
        const invalidStep = () => select(orders, (order) => ({
    ...
    test("rejects legacy array selection syntax at runtime", () => {
        const users = createUsersTable();
        expect(() => select(users, (user) => ([user.id] as never))).toThrow(LEGACY_SELECTION_ARRAY_ERROR);
    });
    test("rejects non-canonical built-in dialect names", () => {
        const users = createUsersTable();
        expect(() => toSql(select(users, (user) => ({ id: user.id })), sqlRenderer({ dialect: "PostgreSQL" }))).toThrow(NON_CANONICAL_POSTGRES_DIALECT_ERROR);
    });
