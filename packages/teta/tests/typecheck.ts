...
// @ts-expect-error prepared bindings reject wrong value types
toSqlResult(preparedUsers, { params: { id: "1", minimumId: 0 } });
// @ts-expect-error param types require runtime descriptor evidence
param("id");
...
const stringParamExpr = param("some_string", t.string());
const numberParamExpr = param("some_number", t.float());
...
const uuidFilteredProfiles = pipe(profiles, filter((profile) => eq(profile.id, param("profile_id", t.uuid()))));
...
const parsedDate = dateParse("2024-01-02", "%Y-%m-%d", "date");
const nullableParsedDate = dateParse(nullableParamExpr, "%Y-%m-%d", "date");
const eventDateTruncated = dateTrunc(events.columns.event_date, "day");
const nullableEventTimestampTruncated = dateTrunc(events.columns.event_ts, "day");
...
type _EventDateTruncated = Expect<Equal<ExprType<typeof eventDateTruncated>, SqlDate>>;
type _NullableEventTimestampTruncated = Expect<Equal<ExprType<typeof nullableEventTimestampTruncated>, SqlTimestamp | null>>;
type _ParsedDate = Expect<Equal<ExprType<typeof parsedDate>, SqlDate>>;
type _NullableParsedDate = Expect<Equal<ExprType<typeof nullableParsedDate>, SqlDate | null>>;
...
type _NumberParamExpr = Expect<Equal<ExprType<typeof numberParamExpr>, SqlFloat>>;
...
type _NullableParamExpr = Expect<Equal<ExprType<typeof nullableParamExpr>, SqlString | null>>;
...
type _AggregatedTotals = Expect<Equal<ExprType<typeof aggregatedTotals.columns.totals>, readonly SqlFloat[]>>;
