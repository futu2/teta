...
describe("toSql(query, options)", () => {
    ...
    test("aliases a computed join column selected by a top-level map", () => {
        ...
        const query = pipe(
            ...
            join(
                history,
                left(
                    (account, historic) => eq(account.id, historic.account_id),
                    (account, historic) => ({
                        id: account.id,
                        was_seen: isNotNull(historic.account_id),
                    })
                )
            ),
            map(pick("id", "was_seen"))
        );
        expect(toSql(query, { dialect: "postgresql", format: "compact" })).toBe(
            "SELECT accounts_0.id, history_1.account_id IS NOT NULL AS was_seen FROM accounts AS accounts_0 LEFT JOIN history AS history_1 ON accounts_0.id = history_1.account_id"
        );
    });
    ...
    test("renders trino unnest as left join with ordinality", () => {
        const sessions = table("sessions", {
            id: t.int(),
            tags: t.array(t.string()),
        });
        const query = pipe(sessions, unnest((session) => session.tags, { value: "tag", ordinality: "idx" }, { outer: true }));
        expect(toSql(query, { dialect: "trino", format: "compact" })).toBe("SELECT sessions_0.id AS id, sessions_0.tags AS tags, unnest_1.tag AS tag, unnest_1.idx AS idx FROM sessions AS sessions_0 LEFT JOIN UNNEST(sessions_0.tags) WITH ORDINALITY AS unnest_1(tag, idx) ON TRUE");
    });
